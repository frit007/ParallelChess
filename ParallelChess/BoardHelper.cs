...
namespace ParallelChess {
    ...
    public static class BoardHelper {
        ...
        public static Winner detectWinnerAreThereValidMoves(Board board, bool areThereValidMoves) {
        ...
        public static bool hasInsufficientMaterialOrTimeLimit(Board board) {
            return board.HalfTurnCounter == 50 || detectInsufficientMaterial(board);
        }

        public static Winner detectWinner(Board board, IEnumerable<Move> moves) {
        ...
        public static bool detectInsufficientMaterial(Board board) {
        ...
        public static void CopyBoard(Board fromBoard, Board toBoard) {
        ...
        public static bool PieceBelongsToMe(Board board, Piece piece) {
        ...
        public static MoveOption CanITakeSquare(Board board, int position) {
        ...
        public static bool Attacked(Board board, int position, byte pretendToBeWhite) {
        ...
        public static bool CanICastleQueenSide(Board board) {
        ...
        public static bool CanCastleKingSide(Board board) {
            CastlingBits castlingBits = board.CastlingBits;
        ...
        private static void AddPawnMove(Board board, int fromPosition, int targetPosition, MoveFlags move, List<Move> moves) {
            Piece takenPiece;
            int row = BoardHelper.PositionRow(targetPosition);
        ...
        private static void AddMove(Board board, int fromPosition, int targetPosition, MoveFlags moveBits, List<Move> moves) {
        ...
        private static void WalkRelativePaths(Board board, int fromPosition, int[] movePositions, List<Move> moves) {
        ...
        public static List<Move> GetMoves(Board board, List<Move> moves = null) {
            if (moves == null) {
                moves = new List<Move>();
            }
        ...
        public static List<Move> GetMovesForPosition(Board board, int fromPosition, List<Move> moves = null) {
            if (moves == null) {
                moves = new List<Move>();
            }
            ...
            switch (justPiece) {
                case Piece.PAWN:
                    ...
                    // We don't need to check if there is a next row is outside the board.
                    // because the pawn is never able to stand on the last because of promotion
                    if (column != BoardHelper.H_COLUMN) {
                    ...
                    // We don't need to check if there is a next row is outside the board.
                    // because the pawn is never able to stand on the last because of promotion
                    if (column != BoardHelper.A_COLUMN) {
                        int move = RelativePosition(fromPosition, -1, direction);
                        bool isEnpassant = board.EnPassantTarget == move;
                        if (
                            // targetposition has to either be the enpassant square.
